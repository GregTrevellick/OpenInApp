// ------------------------------------------------------------------------------
// <auto-generated>
//     This file was generated by VSIX Synchronizer
// </auto-generated>
// ------------------------------------------------------------------------------
namespace OpenInGimp
{
    using System;
    
    /// <summary>
    /// Helper class that exposes all GUIDs used across VS Package.
    /// </summary>
    internal sealed partial class PackageGuids
    {
        public const string guidOpenInGimpPackageString = "81f42674-2915-4a51-8960-f29f9d89bd02";
        public static Guid guidOpenInGimpPackage = new Guid(guidOpenInGimpPackageString);

        public const string guidOpenInGimpCmdSetString = "6a5ff9b8-8e55-48f7-8263-511a3db79208";
        public static Guid guidOpenInGimpCmdSet = new Guid(guidOpenInGimpCmdSetString);

        public const string guidImagesString = "e8f1a2cc-90c3-43e8-92f0-4bde463f4a18";
        public static Guid guidImages = new Guid(guidImagesString);
    }
    /// <summary>
    /// Helper class that encapsulates all CommandIDs uses across VS Package.
    /// </summary>
    internal sealed partial class PackageIds
    {
        public const int GroupOpenInAppCodeWin = 0x1030;
        public const int CmdIdOpenInAppCodeWin = 0x0100;
        public const int GroupOpenInAppItemNode = 0x1040;
        public const int CmdIdOpenInAppItemNode = 0x0110;
        public const int GroupOpenInAppFolderNode = 0x1050;
        public const int CmdIdOpenInAppFolderNode = 0x0120;
        public const int GroupOpenInAppProjNode = 0x1060;
        public const int CmdIdOpenInAppProjNode = 0x0130;
        public const int bmpPic1 = 0x0001;
    }
}